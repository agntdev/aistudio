...
export default function TrainUploadPage() {
  ...
  const checkNSFW = async (
    file: File,
  ): Promise<{ isNSFW: boolean; confidence: number; className: string }> => {
    try {
      ...
      const nsfwPred = predictions.find(
        (p: { className: string; probability: number }) =>
          nsfwClasses.includes(p.className),
      );
  ...
  const uploadViaPresignedURLs = async () => {
    ...
    try {
      ...
      toast.success(`Uploaded ${successCount} files via presigned URLs`, {
        description:
          "In production the backend would now be notified to start training.",
      });
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="mb-10">
          ...
          <p className="text-xl text-zinc-400 mt-2">
            15–30 high-quality selfies. We’ll run face verification, NSFW
            filtering, and LoRA training.
          </p>
        </div>
          ...
          <CardContent className="p-0">
            ...
            >
              ...
              <p className="text-sm text-zinc-400 mt-1">
                or click to browse • JPEG or PNG • up to 30 images
              </p>
              <input
                id="file-input"
                type="file"
          ...
          <Card className="border-white/10 bg-zinc-900 mb-8">
            <CardHeader className="flex flex-row items-center justify-between pb-4">
              <div>
                ...
                <CardDescription>
                  {validFiles.length} valid • {files.length - validFiles.length}{" "}
                  need attention
                </CardDescription>
              </div>
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                {files.map((item, index) => (
                  <div
                    key={index}
                    className="group relative rounded-xl overflow-hidden border border-white/10 bg-zinc-950"
                  >
                    <img
                      src={item.preview}
                      alt={item.file.name}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Card className="border-white/10 bg-zinc-900">
              <CardHeader>
                ...
                <CardDescription>
                  Resolution, format & size checks run in the browser
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">
                    {MIN_RESOLUTION}×{MIN_RESOLUTION}px
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Max file size</span>
                  <span className="font-mono">{MAX_FILE_SIZE_MB} MB</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">{MAX_IMAGES}</span>
                </div>
            ...
            <Card className="border-white/10 bg-zinc-900">
              <CardHeader>
                ...
                <CardDescription>
                  Production flow — backend generates signed URLs
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="text-sm text-zinc-400">
                  1. Frontend requests presigned URLs from your API
                  <br />
                  2. API returns time-limited S3 PUT URLs (DO Spaces / AWS)
                  <br />
                  3. Browser uploads directly to S3 (no server bandwidth)
                  <br />
                ...
                {zipReady && (
                  <div className="text-xs text-emerald-400">
                    Dataset ZIP ready. In real flow you would upload the ZIP or
                    individual files via the signed URLs above.
                  </div>
                )}
              </CardContent>
            </Card>
        ...
        {files.length === 0 && (
          <div className="text-center text-zinc-500 mt-12">
            Upload 15–30 clear, well-lit selfies of the same person for best
            training results.
          </div>
        )}
      </div>
