...
export function LivenessVerification({
  ...
  const [status, setStatus] = useState<
    "idle" | "requesting" | "recording" | "processing" | "success" | "error"
  >("idle");
  ...
  const startLivenessCheck = async () => {
    ...
    try {
      ...
      const livenessResult = await performLivenessCheckOnVideo(
        videoRef.current,
        {
      ...
      if (livenessResult.verified) {
        ...
        const reason =
          livenessResult.metadata?.rejectionReason || "not enough motion";
  ...
  return (
    <Card className="border-white/10 bg-zinc-900">
      <CardHeader>
        ...
        <CardDescription>
          Prove you’re a real person. This helps prevent fake accounts and
        ...
        {status === "idle" && (
          <div className="text-center py-8">
            ...
            <p className="mb-6 text-sm text-zinc-400">
              ...
              <br />
              No video is stored — only a liveness signal.
            </p>
            ...
            <div className="relative aspect-video bg-black rounded-xl overflow-hidden border border-white/10">
              ...
              <div className="absolute bottom-4 left-4 right-4">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  {status === "recording" &&
                    `Recording liveness check... (${requiredDuration}s)`}
                  {status === "processing" &&
                    "Analyzing motion and facial features..."}
                </div>
              </div>
            </div>
        ...
        {status === "success" && result && (
          <div className="rounded-xl border border-emerald-500/30 bg-emerald-950/30 p-6 text-center">
            ...
            <p className="text-xs text-emerald-400/70 mt-1">
              {result.metadata?.framesAnalyzed} frames analyzed •{" "}
